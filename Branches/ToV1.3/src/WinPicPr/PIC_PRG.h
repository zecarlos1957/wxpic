...
/***************************************************************************/
CPROT uint16_t PicPrg_GetConfigWordMask(void);
...
CPROT void PIC_PRG_Flash14ProgCycle(uint16_t wLoadCmd, uint16_t wData);
CPROT void PIC_PRG_Eprom14ProgCycle(uint16_t w);
CPROT bool PIC_PRG_Erase(int iEraseOptions);
CPROT void PIC_PRG_LoadConf(uint32_t dwDeviceAddress, uint16_t wDataAfterLoadCmd );
CPROT uint16_t PIC_PRG_ReadConf(void);
...
// char *HEX_PRG_ConfigWordToString(uint16_t cfg);
