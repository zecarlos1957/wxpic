...
/*                                                                         */
/*  Purpose:                                                               */
/*     Part 1 of Implementation of MainFrame.h                             */
/*     Includes all the autogenerated code by wxSmith                      */
/*                                                                         */
/*  Author:                                                                */
...
const long MainFrame::ID_CHANGE_SESSION_MENU = wxNewId();
...
const long MainFrame::ID_SESSION_CHOICE = wxNewId();
...
void MainFrame::initAuto (void)
{
    ...
    Connect(ID_CHANGE_SESSION_MENU,wxEVT_COMMAND_MENU_SELECTED,(wxObjectEventFunction)&MainFrame::onChangeSessionMenuItemSelected);
...
void MainFrame::initMore (void)
{
    ...
    Connect(ID_SESSION_CHOICE,wxEVT_COMMAND_CHOICE_SELECTED,(wxObjectEventFunction)&MainFrame::onSessionChoiceSelect);
    ...
    aToolBar->AddSeparator();
    aSessionChoice = new wxChoice(aToolBar, ID_SESSION_CHOICE, wxPoint(24,8), wxDefaultSize, 0, 0, 0, wxDefaultValidator, _T("ID_SESSION_CHOICE"));
    updateSessionList();
    aToolBar->AddControl(aSessionChoice);
    aToolBar->Realize();
    SetToolBar(aToolBar);
...
void MainFrame::onChangeSessionMenuItemSelected(wxCommandEvent& event)
{
    if (TSessionConfig::ShowDialog(this))
        updateAllConfig();
}

void MainFrame::onSessionChoiceSelect(wxCommandEvent& pEvent)
{
    if (TSessionConfig::QuickSelect(TSessionListBuilder::GetSelectedSession(aSessionChoice)))
        updateAllConfig();
    else
    {
        wxMessageBox(wxString::Format(_("The session %s is not available. Use session manager dialog for more information."),
                                      aSessionChoice->GetStringSelection().c_str()));
        updateSessionList();
    }
}
