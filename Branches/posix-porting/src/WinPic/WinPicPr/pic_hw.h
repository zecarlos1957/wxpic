...
{ /* Interface Type and Interface-Type - related PARAMETERS                */
  ...
  uint16_t type;                 // a PIC_INTF_TYPE_xxx - constant
  uint16_t wSeparateVddAndVppControl;  // 0=no, 1=yes
...
extern wxChar PicHw_sz255LastError[256];
...
CPROT uint16_t COM_GetPicDataBit(void);
...
CPROT void PIC_HW_ClockOut( uint16_t data_bit );
