...
void CFG_Init(void)
{
  ...
  Config.sz40DeviceName = new char[44];
  memset( Config.sz40DeviceName, 0, 44 );
  ...
  strncpy( Config.sz40DeviceName , "PIC??????", 9);
  ...
  Config.sz80InterfacePluginDLL =  wxT("");
  Config.sz255MplabDevDir = wxT("");      // don't read DEV-files from MPLAB directly
...
//  IniFile.Read("ProgModeSequence", &PIC_dev_param.iProgModeSequence, 0); // 0=PROGMODE_VDD_THEN_VPP (usually except for DS41173b)
  ...
  strncpy( Config.sz40DeviceName, s.mb_str(wxConvISO8859_1), 40 );
