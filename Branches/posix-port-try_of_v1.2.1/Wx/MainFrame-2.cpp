...
////---------------------------------------------------------------------------
//void MainFrame::FormCreate(TObject *Sender)
...
//    PHWInfo.dwhMainWindow = (long)TheMainFrame->GetHandle();
//    // ( Why we cannot retrieve this handle-thing in PicHW.cpp ?
//    //   Because we don't want to have too much WxWidget-specific stuff in there)
//   PHWInfo.dwhAppInstance = GetWindowLong(PHWInfo.dwhMainWindow,GWL_HINSTANCE);
...
//     IniFile->Write("SpeedButtonsVisible", Pnl_Tools->Visible );
//     IniFile->Write("ToolWinVisible",ToolWin_iVisible);
//     IniFile->Write("ToolWinLeft", ToolWin_iX1);
//     IniFile->Write("ToolWinTop",  ToolWin_iY1);

//     IniFile->Write("Language",YHF_GetLanguage() );
//     IniFile->Write("LanguageTestMode",YHF_iLanguageTestMode );
...
//   ToolWin_iX1 = pIniFile->ReadInteger("ToolWinLeft", ToolWin_iX1);
//   ToolWin_iY1 = pIniFile->ReadInteger("ToolWinTop",  ToolWin_iY1);
//   ToolWin_iVisible = pIniFile->ReadInteger("ToolWinVisible",  ToolWin_iVisible);
//   Pnl_Tools->Visible = pIniFile->ReadInteger("SpeedButtonsVisible", 1 );
...
//   APPL_i32CustomizeOptions = pIniFile->Read("Customize", 0 );
...
//           if(ToolForm)
//            ToolForm->ShowMsg( pszMsg, TWMSG_ERROR );
        }
        Update();
...
//#else
//                        APPL_ShowMsg( APPL_CALLER_PIC_PRG, 0, _("Sorry, PIC18F not supported .") );
//                        ++error_count;
...
//                ToolForm->ShowMsg( sz80Temp, TWMSG_NO_ERROR );
    ...
    if (APPL_iUserBreakFlag)
    {
        ...
        APPL_iUserBreakFlag = 0;
//     if(ToolForm)
//        ToolForm->ShowMsg( _("USER BREAK"), TWMSG_NO_ERROR );
        return false;
    }
...
//         if(ToolForm)
//            ToolForm->ShowMsg( _("Programming OK"), TWMSG_SUCCESS );
            }
            else
            {
...
//            ToolForm->ShowMsg( pszMsg, TWMSG_NO_ERROR );
        ...
        else
        {
            ...
            APPL_ShowMsg( APPL_CALLER_PIC_PRG, 0, pszMsg  );
//      if(ToolForm)
//        ToolForm->ShowMsg( _("Programming error !") , TWMSG_ERROR   );
            Update();
            m_iMessagePanelUsage = MP_USAGE_ERROR;
...
//   if(ToolForm)
//    ToolForm->ShowMsg( _("Verifying..."), TWMSG_NO_ERROR );
    ...
    else // no 'user break' during PROGRAM
    {
        if (error_count==0)
        {
            ...
            APPL_ShowMsg( APPL_CALLER_PIC_PRG, 0, pszMsg );
//       if(ToolForm)
//        ToolForm->ShowMsg( pszMsg, TWMSG_SUCCESS );
            m_iMessagePanelUsage = MP_USAGE_INFO;
        }
        else
        {
...
// FILE *fp;
    ...
    RedAndGreenLedOff();
//  if(ToolForm)
//    ToolForm->ShowMsg( _("Loading..."),  TWMSG_NO_ERROR );
...
//  REd_DataMem->Modified = false;
    ...
    if (CommandOption.WinPic_i32CmdLineOption_OverrideConfigWord>=0)
    {
        ...
        if ( (WORD)PicBuf_GetConfigWord(0) != (WORD)CommandOption.WinPic_i32CmdLineOption_OverrideConfigWord)
        {
            APPL_ShowMsg( APPL_CALLER_PIC_PRG,0,
...
////---------------------------------------------------------------------------
//void MainFrame::LoadSmallPort(void)
//{
//    wxString s;
//
//    aStatusBar->SetStatusText(_("Loading port driver"));
//    Update();
//
//    wxFileName ExeName (wxStandardPaths::Get().GetExecutablePath());
////  s = Application->ExeName; // Contains the file name of the application’s executable file including path information.
//    s = ExeName.GetPath();
...
//        AddTextToLog(wxString::Format(_("Port Access Error: %s, code %d"), SmallPort.GetLastErrorString(), SmallPort.GetLastError()) );
//        wxMessageBox(SmallPort.GetLastErrorString() ,
//                     _("Error - WxPic could not open the port access driver"),
//                     wxICON_EXCLAMATION | MB_OK );
//    }
//    else
//    {
//        if ( CommandOption.WinPic_iTestMode & WP_TEST_MODE_GUI_SPEED )
//            APPL_LogEvent( _("Port access driver opened") );
//    }
//
//} // end LoadSmallPort()
...
////---------------------------------------------------------------------------
//void MainFrame::UpdateToolWindow(void)  // Caution, don't call in FormCreate !
//{
//  if(ToolForm)
//     ToolForm->ShowMsg("",TWMSG_NO_CHANGE);  // show FILENAME again
//} // end MainFrame::UpdateToolWindow()
////---------------------------------------------------------------------------
